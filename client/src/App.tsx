...
function App() {
  return (
    <div className="App">
      <Header />
      {/* <About /> */}
      <BackColor mainHeight={6000} />
      <Main />
      <SkillDesign1 index={0} mainHeight={6000} />
      <SkillDesign1 index={1} mainHeight={6000} />
      <SkillDesign2 index={2} mainHeight={6000} />
    </div>
  );
}
...
function BackColor(props: BackProps) {
  const [bgColor, setBgColor] = useState("rgb(255,255,255)");

  useEffect(() => {
    const handleScroll = () => {
      const scrollY = window.scrollY;
      if (scrollY >= props.mainHeight && scrollY <= 300 + props.mainHeight) {
        const percentage =
          (scrollY - props.mainHeight) /
          (300 + props.mainHeight - props.mainHeight);
        const newColor = `rgb(${255 - percentage * (255 - 37)}, ${
          255 - percentage * (255 - 109)
        }, ${255 - percentage * (255 - 218)})`;
        setBgColor(newColor);
      } else if (
        scrollY >= 3000 + props.mainHeight &&
        scrollY <= 3300 + props.mainHeight
      ) {
        const percentage =
          (scrollY - (3000 + props.mainHeight)) /
          (3300 + props.mainHeight - (3000 + props.mainHeight));
        const newColor = `rgb(${37 - percentage * (37 - 251)}, ${
          109 - percentage * (109 - 98)
        }, ${218 - percentage * (218 - 80)})`;
        setBgColor(newColor);
      } else if (
        scrollY >= 5500 + props.mainHeight &&
        scrollY <= 5800 + props.mainHeight
      ) {
        const percentage =
          (scrollY - (5500 + props.mainHeight)) /
          (5800 + props.mainHeight - (5500 + props.mainHeight));
        const newColor = `rgb(${251 - percentage * (251 - 128)}, ${
          98 - percentage * (98 - 89)
        }, ${80 - percentage * (80 - 227)})`;
        setBgColor(newColor);
      } else if (scrollY < props.mainHeight) {
        setBgColor("rgb(255,255,255)");
      } else if (scrollY < 3000 + props.mainHeight) {
        setBgColor("rgb(37,109,218)");
      } else if (scrollY < 5500 + props.mainHeight) {
...
function SkillDesign1(props: SkillProps) {
  ...
  if (props.index === 0) {
    topLocation = 0 + props.mainHeight;
  } else if (props.index === 1) {
    topLocation = 3000 + props.mainHeight;
  } else if (props.index === 2) {
    topLocation = 5500 + props.mainHeight;
  }
